...
export const configurations = pgTable(
  "configurations",
  {
    ...
    has_chassis_wash_detergent_pump: boolean().notNull().default(false),
    has_chassis_wash_detergent_manual_antifreeze: boolean()
      .notNull()
      .default(false),
  ...
  // Postgres does not auto-index FK columns; user_id backs the scope filters.
  (t) => [index("configurations_user_id_idx").on(t.user_id)],
).enableRLS();
