...
/** Returns true once the offer has been frozen as an immutable as-sold snapshot. */
export function isOfferFrozen(
  snapshot: Pick<OfferSnapshot, "frozen_at"> | null,
): boolean {
  return snapshot != null && snapshot.frozen_at != null;
}

/** Returns true when the offer is stale and the config is not frozen. */
export function isOfferStale(
  snapshot: Pick<OfferSnapshot, "generated_at" | "frozen_at">,
  status: ConfigurationStatusType,
): boolean {
  // A frozen offer is immutable and cannot be regenerated, so a stale warning
  // would be un-actionable. This also covers the IN_TECH_REVIEW window where the
  // config is editable but the offer stays frozen.
  if (isOfferFrozen(snapshot)) return false;
  // Frozen statuses can't be regenerated by anyone, so a stale warning would be
  // un-actionable. SALES_APPROVED is a locked hand-off snapshot like TECH_APPROVED/CLOSED.
  if (
